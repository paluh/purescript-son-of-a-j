...
instance FromSonJ (Unique Number) (Unique Number) where
  load = Coerce

else instance FromSonJ (Unique Int) (Unique Int) where
  load = Coerce

else instance FromSonJ (Unique String) (Unique String) where
  load = Coerce

else instance FromSonJ (Unique Json) (Unique Json) where
  load = Coerce

else instance FromSonJ (Unique Boolean) (Unique Boolean) where
  load = Coerce

else instance (FromSonJ (Unique a) (Unique b)) ⇒ FromSonJ (Unique (Array a)) (Unique (Array b)) where
  load = mapArray load

else instance (RowToList row rl, RowListFromSonJ rl row row') ⇒ FromSonJ (Unique (Record row)) (Unique (Record row')) where
  load = loadObject (RLProxy ∷ RLProxy rl)

else instance (RowToList row rl, RowListFromSonJ rl row row') ⇒ FromSonJ (Unique (Variant row)) (Unique (Variant row'))where
  load = loadVariant (RLProxy ∷ RLProxy rl)

else instance (Generic b rep, GenericFromSonJ a rep b) ⇒ FromSonJ (Unique a) (Unique b) where
...
instance (Newtype b a, SonJ a) ⇒ GenericFromSonJ a (Constructor name (Argument a)) b where
  loadGeneric = Coerce

-- else instance GenericFromSonJ a (Sum a b) where
--

class RowListFromSonJ (rl ∷ RowList Type) (row ∷ Row Type) (row' ∷ Row Type) | rl → row, rl → row' where
...
instance RowListFromSonJ RowList.Nil () () where
  loadObject _ = Coerce
...
-- data X = X Int String
-- 
-- derive instance Generic X _
-- 
-- class Generic a rep ⇐ Fake a rep | a → rep
-- 
-- instance Fake X (Constructor "X" (Product (Argument Int) (Argument String)))
...
-- -- unsafe (Mutator f) = f
-- -- 
-- -- instance Semigroupoid Builder where
